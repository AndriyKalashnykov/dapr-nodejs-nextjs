...
- **GH_ACCESS_TOKEN rotation** — the token leaked via argv before #158 fixed the recurrence vector still needs user-initiated rotation. PR #158 (Makefile env-only form) + claude-config PR #6 (portfolio-wide skill rules) closed the *recurrence* vector, but the leaked credential itself remains valid until manually invalidated. Re-export the new token in the shell once rotated.
- **OTel trace propagation through Dapr invoker** — still a `WARN` probe in `e2e/e2e-test.sh`. Real fix would inject `traceparent` into `DaprClient.invoker.invoke()` headers in `app/web-nextjs/src/services/backend-ts.ts`. Not urgent; gap is documented.

...
- [ ] **brace-expansion 2.x CVE waiver (GHSA-mh99-v99m-4gvg)** — waived in `pnpm-workspace.yaml` `auditConfig.ignoreGhsas` because no fix is reachable: the advisory range (`<=5.0.7`) covers all of 2.x and upstream shipped no 2.x patch (2.1.2 is the last). The only fix is 5.0.8, three majors up, which would drag `minimatch` across majors — the exact jump the bounded overrides prevent. Exposure is 2.1.2 via `minimatch > glob > @opentelemetry/auto-instrumentations-node` (prod tree). **Remove the waiver when** minimatch widens its brace-expansion range to 5.x (the existing 5.x override then covers it) or upstream backports a 2.x fix. The 5.x half was fixed in #446.
- [ ] **sharp CVE waiver (GHSA-f88m-g3jw-g9cj)** — waived in `pnpm-workspace.yaml` `auditConfig.ignoreGhsas`. sharp 0.34.5 is an *optional* dependency of `next@16.2.11` (used only by the Next.js image optimizer); the fix is sharp 0.35.0, but next (latest) declares `optionalDependencies.sharp: ^0.34.5` (`<0.35.0`), so an override would push sharp outside next's own declared range rather than fixing anything. **Remove the waiver when** a Next.js release widens to `^0.35.0` — then drop it and let the existing `Next.js` Renovate group carry the bump.

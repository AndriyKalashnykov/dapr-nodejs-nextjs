...
- [x] **PR #153 — comprehensive hardening pass — MERGED** ([d78d720](https://github.com/AndriyKalashnykov/dapr-nodejs-nextjs/commit/d78d720)) — squash-merged 2026-05-02 with all 9 CI jobs green. Final root cause: `app/backend-ts/dapr/components/subscriptions.yaml` had `apiVersion: dapr.io/v2alpha`; Dapr 1.17.6 only recognises `dapr.io/v2alpha1`. Silent 0-subscription load was masking pub/sub for a long time — the new probe was the first thing to exercise the round-trip end-to-end. Trace-propagation probe stays WARN (separate OTel-through-Dapr-invoker gap; not addressed here).
...
- [x] **GH_ACCESS_TOKEN argv exposure fixed (2026-05-02)** — Makefile `ci-run` recipe now passes `--secret GH_ACCESS_TOKEN` (env-only form) instead of `--secret GH_ACCESS_TOKEN=$VALUE`. The previous form put the token value into argv, where `ps -ef` exposed it to any local user. **The previously-leaked token still needs user rotation** — this fix prevents recurrence but does not invalidate the leaked credential.
- [x] **e2e pub/sub probes — apiVersion typo (2026-05-02)** — fixed: `app/backend-ts/dapr/components/subscriptions.yaml` had `apiVersion: dapr.io/v2alpha` (the scaffold template correctly uses `v2alpha1`). Dapr 1.17.6 silently ignored the file because the apiVersion didn't match anything it recognized — `Loading Declarative Subscriptions…` was followed by zero subscription registrations, so the consumer endpoint was never bound to the `todo-data` topic. Both the round-trip and pub/sub-negative probes failed in CI for this reason; the trace-propagation probe remains a separate WARN (OTel context propagation through the Dapr invoker is a known gap). The two pub/sub probes are now restored to FAIL since they're a real signal once the subscription works.

## Session resume — 2026-05-02

End-of-session checkpoint for next session pickup. Working tree was clean at session end on `main`.

**PRs in flight, ordered by impact**:

1. ~~**dapr-nodejs-nextjs#158** — `make ci-run` env-only `--secret KEY` form.~~ **Merged 2026-05-02 (e798e4e).** Critical secret-leak recurrence vector closed.
2. **claude-config#4** — argv secret-leak rules across skills + global rule. Branch `fix/argv-secret-leak-rules`. Adds `/makefile` §10 rule #9 (forbidden argv-leak patterns + detection grep), rewrites `/makefile` §"Local CI with act" examples to bash-array env-only form, replaces Java/Maven and Java/Gradle ci-run inline examples, adds `/ci-workflow` new §"Secret handling in workflow steps" with three classes of leak (argv, echo/log, persistence), patches `/ci-workflow` GOPRIVATE row, and adds a global BLOCKING rule in `rules/common/security.md`. Cross-references between all three.
3. **claude-config#5** — 4 cherry-picked substantive skill commits (replaces closed PR #3). Branch `feat/skills-path-filter-mermaid-12factor`. Picks: `bb50e8f` Path-Filter Strategy elevated to BLOCKING, `cad8907` mermaid-cli pre-pull with retry, `c0bf52f` mermaid-lint :ro mount, `c49d563` 12-factor cross-references in 4 specialist skills. All clean auto-merges. PR #3 was closed because it was opened from the long-running `feat/harden-image-pipeline-paketo-ghcr` branch which couldn't fast-forward (PR #1's path-filter changes had landed on `claude-config/main` in the meantime).

**Ordering note**: claude-config #4 and #5 both touch `commands/ci-workflow.md` and `commands/makefile.md` in different sections. Whichever lands second will need a trivial rebase (no manual conflict expected).

**Outside the PRs**:
- **GH_ACCESS_TOKEN rotation**: in flight in a separate user session (NOT my concern to resume). The previously-leaked token must be invalidated before any session uses `make ci-run` again. PR #158 has merged (closes the recurrence vector); after a new token is issued, re-export `GH_ACCESS_TOKEN` in the shell.
- **OTel trace propagation through Dapr invoker**: still a `WARN` probe in `e2e/e2e-test.sh` — separate gap from PR #153's scope. Real fix would need to inject `traceparent` into `DaprClient.invoker.invoke()` headers in `app/web-nextjs/src/services/backend-ts.ts`. Not urgent.
- **Long-running `feat/harden-image-pipeline-paketo-ghcr` branch on claude-config**: still exists; its substantive commits are now in PR #5 cherry-picks. Auto-sync may continue pushing to that branch — expect new auto-syncs to accumulate but no new substantive content. Don't delete it without coordinating with whatever runs the auto-sync.

**Skill-audit followup**: the 12-factor cross-references in PR #5's `c49d563` reference a "12-Factor Compliance Reference" section in `/project-review`. That section already landed on `claude-config/main` via PR #2's earlier merge of the long-running branch. Verify the cross-references resolve once PR #5 lands.

...
make static-check   # PR-path gate — check-toolchain-alignment + check-cve-waiver-parity + lint + mermaid-lint + diagrams-check + secrets + deps-prune-check (no CVE scan)
...
3. **static-check** (`make static-check`, depends on changes): PR-path gate — `check-toolchain-alignment` + `check-cve-waiver-parity` + `lint` + `mermaid-lint` + `diagrams-check` + `secrets` (gitleaks) + `deps-prune-check`. **No CVE scan** — CVE scanning is tag-gated (see job 8), which is why `check-cve-waiver-parity` exists: it is the offline half that *can* run here
4. **test** (`make backend-test`, depends on build): backend unit tests with coverage
5. **integration-test** (`make backend-test-integration`, depends on build): Postgres **and Redis** services + slim Dapr sidecar with `redis-state` + `redis-pubsub` components, so the read-through-cache and pub/sub-publish integration tests actually execute (they skip-guard to green without a reachable Redis-backed component)
6. **web-nextjs** (`make web-nextjs-ci`, depends on changes): lint + Vitest + Next.js production build
7. **e2e** (**tag pushes only**; depends on integration-test + web-nextjs): docker compose build/up/test/down via `e2e/e2e-test.sh`, then `make web-nextjs-integration` (Next route → Dapr → backend → Postgres) + `make e2e-browser` (Playwright SSR round-trip) against the running stack, then an OWASP ZAP baseline **DAST** scan (`make dast`). **All of these run ONLY at release tags** — PRs and `main` pushes skip the whole job, and `main-rot.yml` does NOT run e2e/DAST/browser, so there is **no per-PR or per-`main` full-stack, browser, or DAST coverage** (deliberate trade-off for PR speed). `ci-pass` stays green on non-tag runs because a skipped job is neither failure nor cancelled (same as cve-check/docker). Note: `docker` (the GHCR publish) does NOT `need` e2e, so at a tag the image is built/pushed/signed regardless of the e2e/DAST/browser result — the tag-time DAST is **advisory, not artifact-blocking**
8. **cve-check** (**tag pushes only**): `make cve-check` — `pnpm audit` + Trivy **filesystem** scan, run under `make -k` so **both** report every time. (They were plain prerequisites until 2026-07-26, and a `pnpm audit` failure stopping make before Trivy masked a real Trivy breakage for five consecutive main-rot runs.) PRs skip it for speed; `main-rot.yml` runs it daily on `main` so vuln-advisory decay still surfaces within 24h
...
`.github/workflows/main-rot.yml` runs `make -k static-check cve-check` against `main` daily (07:00 UTC) plus on-demand via `workflow_dispatch`. It includes `cve-check` (which the PR/push `static-check` no longer does — CVE scanning is tag-gated) so vuln-advisory decay on `main` still surfaces within 24h. A failure surfaces via GitHub workflow-failure email; no PR required. The `-k` is load-bearing — make stops at the first failing goal, so without it a red `static-check` blanks the CVE arm of the only non-tag run that performs it.
...
- [ ] **brace-expansion 2.x CVE waiver (GHSA-mh99-v99m-4gvg == CVE-2026-14257)** — waived in **both** waiver homes: `pnpm-workspace.yaml` `auditConfig.ignoreGhsas` (as the **GHSA** id, for `pnpm audit`) and `.trivyignore.yaml` (as the **CVE** id, purl-pinned to `pkg:npm/brace-expansion@2.1.2`, for every `trivy` surface). Trivy resolves no GHSA→CVE aliases, so the ids differ by necessity; `make check-cve-waiver-parity` keeps the two files in step. No fix is reachable: the advisory range (`<=5.0.7`) covers all of 2.x and upstream shipped no 2.x patch (2.1.2 is the last). The only fix is 5.0.8, three majors up, which would drag `minimatch` across majors — the exact jump the bounded overrides prevent. Exposure is 2.1.2 via `minimatch > glob > @opentelemetry/auto-instrumentations-node` (prod tree). **Remove the waiver (from BOTH files) when** minimatch widens its brace-expansion range to 5.x (the existing 5.x override then covers it) or upstream backports a 2.x fix. The 5.x half was fixed in #446.
- [ ] **sharp CVE waiver (GHSA-f88m-g3jw-g9cj)** — waived in **both** waiver homes: `pnpm-workspace.yaml` `auditConfig.ignoreGhsas` and `.trivyignore.yaml` (purl-pinned to `pkg:npm/sharp@0.34.5`). This advisory has no CVE id, so both files name it identically. sharp 0.34.5 is an *optional* dependency of `next@16.2.11` (used only by the Next.js image optimizer); the fix is sharp 0.35.0, but next (latest) declares `optionalDependencies.sharp: ^0.34.5` (`<0.35.0`), so an override would push sharp outside next's own declared range rather than fixing anything. **Remove the waiver (from BOTH files) when** a Next.js release widens to `^0.35.0` — then drop it and let the existing `Next.js` Renovate group carry the bump. (Re-checked 2026-07-26: latest next is 16.2.12 and still declares `^0.34.5`.)

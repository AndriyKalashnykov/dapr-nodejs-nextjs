...
7. **e2e** (depends on integration-test + web-nextjs): docker compose build/up/test/down via `e2e/e2e-test.sh`, then `make web-nextjs-integration` (Next route → Dapr → backend → Postgres) + `make e2e-browser` (Playwright SSR round-trip) against the running stack, then an OWASP ZAP baseline **DAST** scan (`make dast`)
